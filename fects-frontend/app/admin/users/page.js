...
import { Backend_Server_URL } from '@/components/configurationData/ConfigurationData';

// Change these variables for a new page
const TABLE_HEAD = ["Full Name & Email", "Username", "Phone No.", "Address", "Actions"];
const endpointUrl = `${Backend_Server_URL}/v1/api/appUser/`;
const routeToPage = "/admin/users";

const UsersList = () => {
  const [resourceData, setResourceData] = useState([])
  const [searchText, setSearchText] = useState("")
  const [noSearchData, setNoSeachData] = useState(false)
  ...
  const FetchData = async () => {
    ...
    try {
      ...
      const ErrorMessage = "Error in Fetch User Data ~ " + err.message
  ...
  const HandleDelete = async (id) => {
    try {
      const deleteData = await axios.delete(`${endpointUrl}${id}`);
      setResourceData(resourceData.filter((item) => item.id !== id));
      setSuccess(true)
    } catch (err) {
      const ErrorMessage = "Error in Delete Data ~ " + err.message
      setError(ErrorMessage)
    }
  };
  ...
  const HandleSearch = () => {
    try {
      axios.get(`${endpointUrl}search/${searchText}`)
        .then(response => {
          setResourceData(response.data)
          setNoSeachData(response.data.length === 0);
        })
    } catch (err) {
      const ErrorMessage = "Error in Search Data ~ " + err.message
      setError(ErrorMessage)
    }
  }
  ...
  const EachPageData = resourceData.slice(indexOfFirstData, indexOfLastData);
  const CountPage = Math.ceil(resourceData.length / DataPerPage)
  ...
  return (
    <div className="sm:px-6 w-[1240px] m-auto">
      ...
      <div className="px-4 md:px-10 py-4 md:py-7 mt-20">
        <div className="flex items-center justify-between">
          <p className="focus:outline-none text-base sm:text-lg md:text-xl lg:text-2xl font-bold leading-normal text-gray-800">AVAILABLE USERS</p>
          <button className="focus:ring-2 focus:ring-offset-2 focus:ring-indigo-600 mt-4 sm:mt-0 inline-flex items-start justify-start px-6 py-3 bg-indigo-700 hover:bg-indigo-600 focus:outline-none rounded">
            <Link href={`${routeToPage}/0`} className="text-sm font-medium leading-none text-white">Create New User</Link>
          </button>
        </div>
      </div>
      ...
      <div className='mt-10 mb-5 flex justify-center items-center' >
        <div className="md:w-1/2">
          <div className="relative w-full">
            ...
            <input
              type="text"
              className="block w-full p-2 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg focus:ring-primary-500 focus:border-primary-500  dark:border-gray-600 dark:placeholder-gray-400 dark:text-gray dark:focus:ring-primary-500 dark:focus:border-primary-500"
      ...
      <div className="bg-white py-4 md:py-7 px-4 md:px-8 xl:px-10">
        <div className="mt-7">
          <table className="w-full whitespace-nowrap">
            <thead>
              <tr className='h-16 border border-gray-200 rounded bg-gray-50'>
                {TABLE_HEAD.map((head, index) => (
                  <th
                    key={index}
                    className="cursor-pointer border-y border-blue-gray-100 bg-blue-gray-50/50 transition-colors hover:bg-blue-gray-50 text-base font-medium leading-none text-gray-700"
                  >
                    {head}
                  </th>
                ))}
                    ...
                    <tr className="focus:outline-none h-16 border border-gray-100 rounded hover:bg-gray-100">
                      <td className="px-12 py-3">
                        <div className="w-48 flex items-center gap-3">
                          {
                            data.images.map((imgPath) => (
                              <div key={imgPath}>
                                <img src={imgPath.globalPath} alt={data} className='rounded' width={30} height={30} />
                              </div>
                            ))
                          }
                          <div className="data">
                            <p className="font-normal text-sm text-gray-900">{data.firstName} {data.lastName}</p>
                            <p className="font-normal text-xs leading-5 text-gray-400"> {data.email} </p>
                          </div>
                        </div>
                      </td>

                      <td className="pl-24">
                        <p className="text-sm leading-none text-gray-600 ml-2">{data.username}</p>
                      </td>

                      <td className="pl-5">
                        <p className="text-sm leading-none text-gray-600 ml-2">{data.phone.phoneNumber}</p>
                      </td>

                      <td className="pl-5">
                        <p className="text-sm leading-none text-gray-600 ml-2">{data.address.city}</p>
                      </td>

                      <td className="flex p-5 items-center gap-0.5">
                        <Link href={`${routeToPage}/${data.id}`} >
                          <button className="p-2  rounded-full bg-white group transition-all duration-500 hover:bg-indigo-600 flex item-center">
                            <svg className="cursor-pointer" width="20" height="20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <button onClick={() => HandleDelete(data.id)} className="p-2 rounded-full bg-white group transition-all duration-500 hover:bg-red-600 flex item-center">
      ...
      {error && <h1 className='text-red-500 text-lg text-center mb-10'>{error}</h1>}
      {loading && <h1 className='text-gray-500  text-lg text-center mb-10'>Loading User Data...</h1>}
      {noSearchData && <h1 className='text-gray text-lg text-center mb-10'>User Not Found !!</h1>}
