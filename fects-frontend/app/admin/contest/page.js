...
const TABLE_HEAD = ["Image", "Contest Name", "Contest Description", "Organised By", "Start Date", "Finish Date", "Actions"];
const endpointUrl = `${Backend_Server_URL}/v1/api/contest/`;
const routeToPage = "/admin/contest";
...
const ContestList = () => {
  ...
  const FetchData = async () => {
    try {
      ...
      const ErrorMessage = "Error in Fetch Contest Data ~ " + err.message
    ...
    <div className="sm:px-6 w-[1240px] m-auto">
      ...
      <div className="px-4 md:px-10 py-4 md:py-7 mt-20">
        <div className="flex items-center justify-between">
          <p className="focus:outline-none text-base sm:text-lg md:text-xl lg:text-2xl font-bold leading-normal text-gray-800">CONTEST LIST</p>
          <button className="focus:ring-2 focus:ring-offset-2 focus:ring-indigo-600 mt-4 sm:mt-0 inline-flex items-start justify-start px-6 py-3 bg-indigo-700 hover:bg-indigo-600 focus:outline-none rounded">
            <Link href="/admin/contest/0" className="text-sm font-medium leading-none text-white">Create New Contest</Link>
          </button>
        </div>
      </div>
                        ...
                        <div className="flex items-center">
                          {
                            data.images.map((imgPath, index) => (
                              <img key={index} src={imgPath.globalPath} alt={data.name} className='w-8 h-8 rounded-full' />
                            ))
                          }
                        </div>
                      </td>
                      <td className="px-4 py-2 whitespace-nowrap">
                        <p className="text-sm leading-none text-gray-600">{data.name}</p>
                      </td>

                      <td className="px-4 py-2 whitespace-nowrap">
                        <p className="text-sm leading-none text-gray-600">{data.description}</p>
                      </td>

                      <td className="px-4 py-2 whitespace-nowrap">
                        <p className="text-sm leading-none text-gray-600">{data.createdBy}</p>
                      </td>

                      <td className="px-4 py-2 whitespace-nowrap">
                        <p className="text-sm leading-none text-gray-600">{data.startDate}</p>
                      </td>

                      <td className="px-4 py-2 whitespace-nowrap">
                        <p className="text-sm leading-none text-gray-600">{data.finishDate}</p>
                      </td>
      ...
      {loading && <h1 className='text-gray-500  text-lg text-center mb-10'>Loading Contest Data...</h1>}
      {noSearchData && <h1 className='text-gray text-lg text-center mb-10'>Contest Not Found !!</h1>}
